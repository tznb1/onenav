...
<?php }elseif(getconfig('GoAdmin')  == 'on'  ){?>
    <p class="login"><a href="/index.php?c=<?php if($login =='login'){echo $login;}else{echo getloginC($u);}?>&u=<?php echo $u?>" title="登录" target="_blank">
...
<?php }elseif(getconfig('GoAdmin')  == 'on'  ){?>
    <a href="/index.php?c=<?php if($login =='login'){echo $login;}else{echo getloginC($u);}?>&u=<?php echo $u?>" title="登录" target="_blank"><i class="fa fa-user-circle-o"></i></a>
<?php }?>
</div><!-- 右上角End -->
</nav><!-- 顶部栏End -->
<!--正文内容部分-->
...
<div class="site-type" id="category-<?php echo $category['id']; ?>"><?php echo geticon($category['Icon']).$category['name']; ?><?php echo $property; ?></div>
    ...
    <!-- 遍历链接 -->
		...
		<p class="name">
		<img src="<?php if (getconfig('LoadIcon')  == 'on'  ){echo 'https://favicon.rss.ink/v1/'.base64($link['url']);}else{echo $libs.'/Other/default.ico';} ?>">
		<?php echo $link['title'];?>
		</p>
