<?php  //入口(落幕修改:2022/01/29)
error_reporting(E_ALL^E_NOTICE^E_WARNING^E_DEPRECATED);
require ('./initial/initialization.php');//检查是否存在用户表,不存在则初始化!
...
$u = !empty($u)?$u:(!empty($CookieU)?$CookieU:'admin');//优先级:Get>Cookie>Default
$Duser    = $udb->get("config","Value",["Name"=>'DefaultUser']);//默认用户
...
if(($c == '') or ($c == 'index') or ($c == 'admin')or ($c == 'click')or ($c == 'api')){
...
//echo($userdb['RegIP'].$userdb['RegTime'].$userdb['User'].$userdb['Pass']);
if(!isset($username)||!isset($userdb['ID'])){msg(-1002,'未找到账号数据.');}
elseif(!file_exists($SQLite3) && !copy('./initial/onenav.simple.db3',$SQLite3)){msg(-1003,'创建用户数据库失败.');}//需要写入初始数据
