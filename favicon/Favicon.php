...
class Favicon
{
    ...
    protected function getData()
    {
         ...
         */
        if ($this->data == NULL) {
            $thrurl = '//ico.hnysnet.com/get.php?url=';
            $icon = getFile($thrurl . $this->full_host);
            $this->data = $icon;
        }
        if ($this->data == NULL) {
            //各个方法都试过了，还是获取不到。。。
    ...
    private function getFile($url, $isimg = false, $timeout = 2)
    {
        ...
        $arr = array('status' => 'FAIL', 'data' => '', 'real_url' => '');
        if (!$isimg || $mimeArray[0] == 'image') {
            if ($ret != false) {
                $status = curl_getinfo($ch, CURLINFO_HTTP_CODE);
                $arr = array('status' => $status >= 200 && $status <= 299 ? TRUE : FALSE, 'data' => $ret, 'real_url' => curl_getinfo($ch, CURLINFO_EFFECTIVE_URL));
            }
            curl_close($ch);
            return $arr;
        } else {
